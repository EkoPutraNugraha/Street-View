...
class GPSTracker(private val context: Context) : Service(), LocationListener {
    ...
    fun getLocation() {
        try {
            ...
            if (isGPSTrackingEnabled) {
                locationManager!!.requestLocationUpdates(provider_info!!, MIN_TIME_BW_UPDATES, MIN_DISTANCE_CHANGE_FOR_UPDATES.toFloat(), this)
                if (locationManager != null) {
                    location = locationManager!!.getLastKnownLocation(provider_info!!)
                    updateGPSCoordinates()
                }
            } else {
                Log.e(TAG, "GPS tracking not enabled")
            }
        } catch (e: Exception) {
    ...
    fun updateGPSCoordinates() {
        if (location != null) {
            ...
            Log.d(TAG, "Updated GPS coordinates: Latitude: $latitude, Longitude: $longitude")
        } else {
